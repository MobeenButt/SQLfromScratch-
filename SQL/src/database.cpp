    ...
    const std::string& having_clause)
{
    ...
    for (size_t i = 0; i < table->columns.size(); i++)
    {
        if (table->columns[i].name == group_column)
        {
            group_col_index = i;
            break;
        }
    ...
    if (where_clause.empty())
    {
        all_records = storage_manager->getAllRecords(getTablePath(table_name));
    }
    else
    {
        // Parse where clause and filter
        std::vector<std::string> parts = split(where_clause, ' ');
        if (parts.size() >= 3)
        {
            selectWithCondition(table_name, parts[0], parts[1], parts[2], all_records);
        }
    }
    ...
    {
        std::string key = record.values[group_col_index];
        double value = 0;
        ...
        if (agg_function != "COUNT(*)")
        {
            ...
            for (size_t i = 0; i < table->columns.size(); i++)
            {
                if (table->columns[i].name == agg_column)
                {
                    agg_col_index = i;
                    break;
                }
            }
            value = std::stod(record.values[agg_col_index]);
        }

        // Update group aggregates
        groups[key].group_key = key;
        groups[key].count++;
        if (agg_function.find("SUM(") != std::string::npos ||
            agg_function.find("AVG(") != std::string::npos)
        {
            groups[key].sum += value;
        }
        if (agg_function.find("MIN(") != std::string::npos)
        {
            groups[key].min = std::min(groups[key].min, value);
        }
        if (agg_function.find("MAX(") != std::string::npos)
        {
            groups[key].max = std::max(groups[key].max, value);
        }
    }
    ...
    {
        Record r;
        r.values.push_back(key); // Group key

        if (agg_function == "COUNT(*)")
        {
            r.values.push_back(std::to_string(group.count));
        }
        else if (agg_function.find("SUM(") != std::string::npos)
        {
            r.values.push_back(std::to_string(group.sum));
        }
        else if (agg_function.find("AVG(") != std::string::npos)
        {
            r.values.push_back(std::to_string(group.sum / group.count));
        }
        else if (agg_function.find("MIN(") != std::string::npos)
        {
            r.values.push_back(std::to_string(group.min));
        }
        else if (agg_function.find("MAX(") != std::string::npos)
        {
            r.values.push_back(std::to_string(group.max));
        }

        results.push_back(r);
    }
    if (!having_clause.empty()) {
        std::vector<std::string> having_parts = split(having_clause, ' ');
        if (having_parts.size() >= 3) {
            std::string having_col = having_parts[0];
            std::string having_op = having_parts[1];
            std::string having_val = having_parts[2];
            
            auto it = std::remove_if(results.begin(), results.end(),
                [&](const Record& r) {
                    double record_val = std::stod(r.values[1]); // Aggregate value
                    double compare_val = std::stod(having_val);
                    return !compareValues(std::to_string(record_val), having_val, having_op);
                });
